...
import { manuscriptExcerpt } from "~/utils/manuscript";
...
const emit = defineEmits<{
  localeView: [post: Post];
}>();

watch(view, (next) => emit("localeView", next), { immediate: true });
  ...
  <div class="min-w-0 max-w-full space-y-3 px-4 pb-3 pt-4 sm:px-5">
    ...
    >
      <p class="text-xs font-medium text-slate-600">
        {{
          authorLabel(post.sharedPost.author.name, post.sharedPost.author.email)
        }}
