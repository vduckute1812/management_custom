...
const buckets = computed<Bucket[]>(() => {
  const periods =
    granularity.value === "day" ? 14 : granularity.value === "week" ? 8 : 6;
...
/** Stable string so we avoid deep-watching the tasks array. */
...
async function renderCharts() {
  ...
  const estimated = buckets.value.map((b) => b.estimated);
  const spent = buckets.value.map((b) => b.spent);
  const completed = buckets.value.map((b) => b.completed);
  const rolledOver = buckets.value.map((b) => b.rolledOver);
  ...
  <div class="space-y-5">
    ...
    >
      ...
      <ul class="divide-y divide-slate-100">
        <li v-for="row in epicBreakdown" :key="row.epic.id">
          <NuxtLink
            ...
            <div class="text-right tabular-nums">
              <p class="text-sm font-semibold text-slate-900">
                ...
                <span class="text-slate-400"
                  >{{ row.epic.estimatedHours }}h</span
                >
              </p>
              <p
