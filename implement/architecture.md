...
| Tests      | Vitest + Playwright            | `npm test` (DB-free units) + CI MySQL job (`npm run test:integration`) + Playwright public smoke (`npm run test:e2e`)           |
...
│   ├── app/                         # AppHeader (+ Nav / AccountMenu), AppFooter, CommandPalette, LanguageSwitcher, …
...
**Shared types.** Account identity / roles / OAuth consts live in `types/auth.ts` (`UserRole`, `AuthUser`, `AuthProvider`, …). Task/epic types stay in `types/task.ts`, which **re-exports** the auth surface for backward-compatible imports. Server barrel `server/db/core/types.ts` re-exports auth from `types/auth.ts`. Money domain consts/types use an **explicit named-export** barrel at `types/money.ts` (no `export *`) over `types/money/{direction,categories,…}.ts`.
