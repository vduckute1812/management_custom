...
Wire DTOs: `~/types/post.ts`, `~/types/story.ts`. Domain SQL: `server/db/posts.ts` (list/CRUD), `server/db/postReactions.ts`, `server/db/postComments.ts`, `server/db/stories.ts`, `server/db/uploads.ts`, `server/db/categories.ts`.
