...
**Migrations on disk today:** `0001_initial` → … → `0035_posts_visibility_created_index` → `0036_friendship_acl_updated_indexes` → `0037_chat_message_created_id_index`.
...
| `posts.visibility`                                                                         | `TINYINT UNSIGNED`      | `Public=0, Private=1, Shared=2, Friends=3` (`PostVisibility` in `types/post.ts`)             |
| `friendships.status`                                                                       | `TINYINT UNSIGNED`      | `Pending=0, Accepted=1` (`FriendshipStatus` in `types/friendship.ts`)                        |
...
-- on every successful /api/auth/refresh. `family_id` ties rotations together;
-- presenting a revoked hash revokes the whole family (reuse detection).
...
| Table              | Purpose                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `posts`            | Feed posts: `body`, `format` (`0` update / `1` manuscript), optional `title`, `visibility` (`0` public / `1` private / `2` shared / `3` friends), optional `category_id`, `font_family`, `text_color`, optional `shared_post_id`, optional `translation_group_id`, `content_locale`, denormalized `comment_count` (migration `0016`; integer visibility/format in `0019`; Friends=`3` in `0033`) |
| `post_audience`    | ACL rows for `visibility = 2` (`PostVisibility.Shared`)                                                                                                                                                                                                                                                                                                                                          |
| `friendships`      | Facebook-style friend graph: `requester_id`, `addressee_id`, `status` (`Pending=0` / `Accepted=1`); unique pair; migration `0033` also backfills Normal-user Public posts → Friends                                                                                                                                                                                                              |
...
| Table                   | Notes                                                                                           |
| ----------------------- | ----------------------------------------------------------------------------------------------- |
| `money_user_categories` | `mcat_*`; open-ended `name` / `emoji` / `color`; `direction` int (`Out=0`/`In=1`); soft-archive |
