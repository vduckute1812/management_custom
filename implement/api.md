...
| `GET`    | `/api/admin/articles/pending/settings`       | Pipeline settings `{ dailyFetchEnabled }` (persisted in `app_settings`).                                                                                                                                                                                                                                                                                                                                               |
| `PATCH`  | `/api/admin/articles/pending/settings`       | Body `{ dailyFetchEnabled: boolean }` — toggle the daily automatic fetch job. Returns `{ ok: true, dailyFetchEnabled }`.                                                                                                                                                                                                                                                                                               |
| `POST`   | `/api/admin/articles/pending/bulk-delete`    | Body `{ ids: string[] }`. Hard-deletes pipeline rows (any status). Approved rows also remove the published feed post when `published_post_id` is set. Returns `{ ok: true, deleted, postsDeleted }`.                                                                                                                                                                                                                   |

Admin UI: `/admin/articles/pending` (filters, settings toggle, bulk delete, “Fetch now” with `force: true`) and `/admin/articles/pending/:id` (original vs rewrite panels, preview, use-original, regenerate with poll, approve → `/feed`).
...
| Method   | Endpoint                      | Auth     | Description                                                                                                                                                                                                          |
| -------- | ----------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/api/friends`                | Required | Compact overview `{ friends, incoming, outgoing, incomingTotal, nextCursors: { friends, incoming, outgoing } }`. Query: `limit` (1–100, default 50), optional `friendsCursor` / `incomingCursor` / `outgoingCursor`. |
| `GET`    | `/api/friends/accepted`       | Required | Cursor page `{ rows, nextCursor }`. Query: `limit` (1–100, default 50), optional `cursor`.                                                                                                                           |
| `GET`    | `/api/friends/incoming`       | Required | Cursor page of pending requests addressed to you (`{ rows, nextCursor }`).                                                                                                                                           |
| `GET`    | `/api/friends/outgoing`       | Required | Cursor page of pending requests you sent (`{ rows, nextCursor }`).                                                                                                                                                   |
