...
Signed-in 1:1 messaging. Spec: [`chat-spec.md`](./chat-spec.md). Tables: migrations `0013_chat` + `0014_chat_media` + `0015_chat_unread_counters`. Client page: `/chat` uses SSE for the open thread (`…/stream`) with a slow REST fallback if the stream drops.
